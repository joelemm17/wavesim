...
class wave_simulation_AI:
    ...
    def run(self):
        ...
        writer = imageio.get_writer(
            '/Your_out_path_goes_here/test.mp4', fps=30)
        ...
        for t in range(0, self.steps):
            ...
            u_array_v = u_array_v + \
                np.multiply(np.square(c), self.Laplace(
                    u_array, self.dx) * self.dt)
            u_array = u_array + u_array_v * self.dt

            arr_new = u_array
            arr_new[b_el_mask == 1] = 0

            arr_new_r = np.maximum(arr_new, 0) / \
                np.max(np.maximum(arr_new, 0) + 1e-10)
            arr_new_g = np.minimum(arr_new, 0) / \
                np.min(np.minimum(arr_new, 0) + 1e-10)
            arr_new_b[b_el_mask == 1] = 1

            arr_new_b[self.Edge_detect(c) == 1] = 1

            outputdata[0:self.sz_x, 0:self.sz_y,
                       0] = arr_new_r[0:self.sz_x, 0:self.sz_y] * 255
            outputdata[0:self.sz_x, 0:self.sz_y,
                       1] = arr_new_g[0:self.sz_x, 0:self.sz_y] * 255
...
#         c[(542-round(m.pow(m.pow((1024-200)/2, 2) -
#            m.pow(-(y-100)+((1024-200)/2), 2), 0.5))):542, y] = c_const * 0.6
...
#         y_full = m.pow(m.pow((1024 - 200) / 2, 2) -
#                        m.pow(-(y - 100) + ((1024 - 200) / 2), 2), 0.5)
...
#         broadcast_el_mask[5:15, int(
#             1024/2-5+my_lambda/2*(a-n/2)):int(1024/2+5+my_lambda/2*(a-n/2))] = 1
#         broadcast_el[5:15, int(1024/2-5+my_lambda/2*(a-n/2)):int(1024 /
#                                                                  2+5+my_lambda/2*(a-n/2))] = m.sin(2 * m.pi * t * 0.010)
...
#             phase = 2 * m.pi / 4 * a
#             phase = 0
#             broadcast_el_mask[int(1024/2-1):int(1024/2+1), int(1024/2-1+my_lambda/4*(
#                 a)-my_lambda/4*n/2):int(1024/2+1+my_lambda/4*(a)-my_lambda/4*n/2)] = 1
#             broadcast_el[int(1024/2-1):int(1024/2+1), int(1024/2-1+my_lambda/4*(a)-my_lambda/4*n/2):int(
#                 1024/2+1+my_lambda/4*(a)-my_lambda/4*n/2)] = m.sin(2 * m.pi * (t) * 0.008 + phase)
...
#             phase = 2 * m.pi * \
#                 m.sqrt(m.pow(500, 2)-m.pow(my_lambda/2*(a-n/2), 2))/my_lambda

#             broadcast_el_mask[5:15, int(
#                 1024/2-5+my_lambda/2*(a-n/2)):int(1024/2+5+my_lambda/2*(a-n/2))] = 1
#             broadcast_el[5:15, int(1024/2-5+my_lambda/2*(a-n/2)):int(
#                 1024/2+5+my_lambda/2*(a-n/2))] = m.sin(2 * m.pi * (t) * 0.008 + phase)
...
#             phase = -2 * m.pi * \
#                 m.sqrt(m.pow(400, 2)-m.pow(my_lambda/2*(a-n/2), 2))/my_lambda

#             broadcast_el_mask[105:115, int(
#                 1024/2-5+my_lambda/2*(a-n/2)):int(1024/2+5+my_lambda/2*(a-n/2))] = 1
#             broadcast_el[105:115, int(1024/2-5+my_lambda/2*(a-n/2)):int(
#                 1024/2+5+my_lambda/2*(a-n/2))] = m.sin(2 * m.pi * (t) * 0.008 + phase)
...
#                 1024/2-5+my_lambda/2*(a-n/2)):int(1024/2+5+my_lambda/2*(a-n/2))] = 1
#             broadcast_el[5:15, int(1024/2-5+my_lambda/2*(a-n/2)):int(
#                 1024/2+5+my_lambda/2*(a-n/2))] = m.sin(2 * m.pi * (t) * 0.024 + phase)
...
# waveguide_img = imageio.imread(
#     './kepler_telescope.bmp')
